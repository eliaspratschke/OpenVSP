...
void VORTEX_TRAIL::InducedVelocity(double xyz_p[3], double q[3], double CoreSize)
{
   ...
   //if(2. * Sigma_ - 0.001 > CoreSize ){printf("%2.10f   %2.10f, \n", CoreSize, 2.*Sigma_ );}
    //printf("%2.10f   %2.10f, \n", CoreSize, 2.*Sigma_ );

    //printf("coresize at Induced velocity level: %2.10f", CoreSize_);
...
void VORTEX_TRAIL::InducedVelocity_(double xyz_p[3], double q[3])
{
   ...
   // Start at the coarsest level .... presumably here the core is irrelevant, since we are far away anyway (my comment after the dots).
...
void VORTEX_TRAIL::CalculateVelocityForSubVortex(VSP_EDGE &VortexEdge, double xyz_p[3], double q[3])
{
   ...
   if ( !VortexEdge.ThereAreChildren() || Ratio >= FarAway_ ) {
      ...
      if (VortexEdge.T() != 0){printf("nonzero member T  \n\n");}

       //printf("corewidth at subvortex level: %2.10f", CoreSize_);
...
double VORTEX_TRAIL::UpdateWakeLocation(int a)
{

    int i, j, m, Level;
    double Vec[3], Mag, dx, dy, dz, dS, MaxDelta, Relax, discrepancy, dist[3], distance;
    ...
    Streamliney = fopen("Streamliney.txt", "a");
    Streamlinez = fopen("Streamlinez.txt", "a");
    //char buffer[100];
*/
    ...
    if(a < start){Relax = 0.85;}
    else{
        argument = - c * (a - start);
        Relax = 1 * exp(argument);
        //if(Relax < 0.1){Relax = 0.1;}
    }
    */

    Relax = 0.85;
   // printf("wake:%i", VSP_VLM_.CurrentWakeIteration_);
    ...
    for ( i = 1 ; i <= NumberOfSubVortices() + 1 ; i++ ) {
        ...
        /*
        if ( i > 1 && i < NumberOfSubVortices() + 1 ) {     //adam bashfort weights.

            Vec[0] = ( -0.5 * VortexEdgeVelocity(i-1)[0] + 1.5*VortexEdgeVelocity(i)[0] );
            Vec[1] = ( -0.5 * VortexEdgeVelocity(i-1)[1] + 1.5*VortexEdgeVelocity(i)[1] );
            Vec[2] = ( -0.5 * VortexEdgeVelocity(i-1)[2] + 1.5*VortexEdgeVelocity(i)[2] );
       ...
       NodeList_[i+1].z() += Relax*dz;
        ...
        /*
       //Streamlinex = fopen("Streamlinex.txt", "a");
       fprintf(Streamlinex, "%2.15f,", NodeList_[i].x());
       //Streamliney = fopen("Streamliney.txt", "a");
       fprintf(Streamliney, "%2.15f,", NodeList_[i].y());
       //Streamlinez = fopen("Streamlinez.txt", "a");
       fprintf(Streamlinez, "%2.15f,", NodeList_[i].z());


       if(i == NumberOfSubVortices() +1){
           fprintf(Streamlinex, "%2.15f\n", NodeList_[i].x());
           fprintf(Streamliney, "%2.15f\n", NodeList_[i].y());
           fprintf(Streamlinez, "%2.15f\n", NodeList_[i].z());
       }
*/
       MaxDelta = MAX((dx*dx + dy*dy + dz*dz)/(S_[0][i+1]*S_[0][i+1]), MaxDelta);
        /*
       dist[0] = NodeList_[i+1].x() - NodeList_[i].x();
       dist[1] = NodeList_[i+1].y() - NodeList_[i].y();
       dist[2] = NodeList_[i+1].z() - NodeList_[i].z();

       discrepancy = sqrt(vector_dot(dist, dist));
        */
       if ( DoGroundEffectsAnalysis_ ) NodeList_[i+1].z() = MAX(NodeList_[i+1].z(), 0.);
       /*
       distance = ABS(dS) - ABS(distance);
       printf("disc= %2.10f", discrepancy);
...
//opens files as append, so delete streamline files for every new case.
    ...
    for (int i = 1 ; i <= NumberOfSubVortices() + 1 ; i++ ) {
